...
def run_zoxide() -> str | None:
    try:
        ...
        emit("sesh: zoxide command not found", stderr=True)
        return None
    except subprocess.CalledProcessError as exc:
        emit(f"sesh: failed to query zoxide (exit code {exc.returncode})", stderr=True)
        return None
...
def select_session(candidates: str) -> tuple[str, int]:
    try:
        ...
        emit("sesh: fzf command not found", stderr=True)
        return "", 1
...
def session_data(session_path: str, session_name: str) -> str | int:
    ...
    try:
        ...
        emit(f"sesh: failed to read template file ({exc})", stderr=True)
        return 1
...
def ensure_session_file(
    ...
    try:
        ...
        emit(f"sesh: failed to write session file ({exc})", stderr=True)
        return 1
...
def goto_session(session_file: Path) -> int:
    try:
        ...
        emit("sesh: kitten command not found", stderr=True)
        return 1
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        emit(f"sesh: failed to create session directory ({exc})", stderr=True)
        return 1
