			...
			vim.iter(vim.split(path, "/"))
				:map(function(segment)
					if segment == vim.fn.fnamemodify(path, ":t") then
						...
						if ok then
							local icon = icons.get_icon("filetype", filetype)
							local file_status =
...
local function set_winbar(bufnr)
	...
	then
		vim.wo[win].winbar = "%{%v:lua.require'config.winbar'.render()%}"
	else
		vim.wo.winbar = ""
	end
end
...
function M.setup()
	...
	vim.api.nvim_set_hl(0, "WinbarDir", { bg = bg, fg = colors.blue, bold = true, italic = true })
	vim.api.nvim_set_hl(0, "WinbarFile", { bg = bg, fg = colors.blue })
